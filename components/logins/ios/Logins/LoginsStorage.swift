...
open class LoginsStorage {
    ...
    open func migrateLogins(newDbPath: String, newDbEncKey: String, sqlCipherDbPath: String, sqlCipherEncKey: String,
        sqlCipherSalt: String) throws -> String {
        return try queue.sync {
            // last param is the "salt" which is only used on iOS.

            return try self.migrateLogins(newDbPath: newDbPath, newDbEncKey: newDbEncKey, sqlCipherDbPath: sqlCipherDbPath,
            sqlCipherEncKey: sqlCipherEncKey, sqlCipherSalt: sqlCipherSalt)
        }
    }
